DDPProvider = {
  ...
  ignoringMethodIds: {},
  ignoringSubscriptions: {
    "kadira.debug.client.init": true
  },
  ignoringSubscriptionIds: Object.create(null)
};
...
DDPProvider._livedata_data = function(original) {
    ...
    switch(msg.msg) {
      case "ready": 
        ...
        _.each(info.subs, function(sub) {
          if (!DDPProvider.ignoringSubscriptionIds[sub]) {
  ...
  function trackLiveUpdates(msg) {
    if (msg.collection === ' __kdconfig' || msg.collection === 'kdInfo') {
      return;
    }
  ...
  return function(msg) {
    ...
    switch(msg.msg) {
      case "method":
        ...
        break;
      case "sub":
        if (DDPProvider.ignoringSubscriptions[msg.name]) {
          DDPProvider.ignoringSubscriptionIds['' + msg.id] = true;
        }
        info.name = msg.name;
        info.id = msg.id;
        eventName = "ddp-sub";
